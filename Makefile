...
# Prefer pkg-config; fall back to sdl2-config + SDL2_image + SDL2_ttf
PKG_CONFIG := $(shell command -v pkg-config 2>/dev/null)
ifdef PKG_CONFIG
  SDL_CFLAGS  := $(shell pkg-config --cflags SDL2 SDL2_image SDL2_ttf 2>/dev/null)
  SDL_LDFLAGS := $(shell pkg-config --libs SDL2 SDL2_image SDL2_ttf 2>/dev/null)
endif
ifndef SDL_CFLAGS
  SDL_CFLAGS  := $(shell sdl2-config --cflags 2>/dev/null) -I/opt/homebrew/include/SDL2 -I/usr/local/include/SDL2
  SDL_LDFLAGS := $(shell sdl2-config --libs 2>/dev/null) -L/opt/homebrew/lib -L/usr/local/lib -lSDL2_image -lSDL2_ttf
endif
